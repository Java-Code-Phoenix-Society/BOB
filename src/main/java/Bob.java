 ...
 * <p>
 * <p>
 * Note: The gui is created in Symantec Visual Caf� while the rest
 * of the program is made in Microsoft Visual J++
 */
public class Bob extends JFrame {
    ...
    Button quitter;
    Button RC;
    Picture picture; //the actual panel wher the very nice picture is displayed
    String currAnswer = new String();
    String ans;
     ...
     * ---------------------------------------------------
     * METHOD: main
     * PURPOSE: Support application format for the program
     * <p>
     * ---------------------------------------------------
     */
    ...
    ---------------------------------------------------
        METHOD: init
        PURPOSE: initialize GUI elements and misc varibles
    ---------------------------------------------------
    ...
    public void init() {
        ...
        out.reshape(7, 15, 608, 120);
        add(out);
        send = new Button("Send");
        send.reshape(7, 293, 112, 30);
        add(send);
        quitter = new Button("Quit");
        quitter.reshape(126, 293, 118, 30);
        ...
        RC.reshape(7, 338, 238, 28);
        add(RC);
        in = new TextArea();
        in.reshape(7, 150, 611, 118);
        add(in);
        picture = new Picture(bobImage);
        picture.setLayout(null);
        picture.reshape(287, 278, 331, 90);
        add(picture);
        add(quitter);
...
// done with the gui. now to the real stuff
     ...
     * ---------------------------------------------------
     * METHOD: action
     * PURPOSE: Create a command-chain based on user interaction.
     * ---------------------------------------------------
     */
    public boolean action(Event evt, Object arg) {
        ...
        if (evt.target instanceof Button || evt.target instanceof TextArea) {
            ...
            if ((this.in.getText().toLowerCase()).equals("quit") || arg.toString().equals("Quit")) System.exit(0);
     ...
     * ---------------------------------------------------
     * METHOD: doChain
     * PURPOSE: The main command loop
     * ---------------------------------------------------
     */

    public void doChain(String txt) {

        Conversation.append("U: " + txt + "\n");
     ...
     * ---------------------------------------------------
     * METHOD:answer
     * PURPOSE: Format the program answer in a good way and
     * type it out.
     * ---------------------------------------------------
     */
     ...
     * ---------------------------------------------------
     * METHOD: printConv
     * PURPOSE: Print the entire conversation into a separate window.
     * ---------------------------------------------------
     */
